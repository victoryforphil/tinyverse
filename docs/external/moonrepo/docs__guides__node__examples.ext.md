----
## External Docs Snapshot // moonrepo

- Captured: 2026-02-17T03:11:54.185Z
- Source root: https://moonrepo.dev/docs
- Source page: /docs/guides/node/examples
- Keywords: moonrepo, docs, monorepo, task runner, toolchain, guides, node, examples
- Summary: [📄️ AngularIn this guide, you'll learn how to integrate Angular into moon.](/docs/guides/examples/angular)
----

Source: https://moonrepo.dev/docs/guides/node/examples

[📄️ AngularIn this guide, you'll learn how to integrate Angular into moon.](/docs/guides/examples/angular)

----
## Notes / Comments / Lessons

- Collection method: sitemap-first discovery scoped to moonrepo docs.
- Conversion path: direct HTML fallback parser.
- This file is one page-level external snapshot in markdown `.ext.md` format.
----
